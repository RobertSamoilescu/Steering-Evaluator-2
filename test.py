...
def test_video(path, time_penalty=6, translation_threshold=1.5, rotation_threshold=0.2, verbose=True):
    ...
    pred_course, toutput = make_prediction(prev_frame, frame, speed)
    ...
    with torch.no_grad():
        for idx in tqdm(itertools.count()):
            ...
            if real_course is not None:
                ...
                full_img = visualisation(process_frame(prev_frame), real_course_distribution, toutput, 1, imgs_path).astype(np.uint8)
