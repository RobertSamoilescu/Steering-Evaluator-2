...
class AugmentationEvaluator:
    ...
    def reshape(self, frame):
        frame = cv2.resize(frame, (256, 128), interpolation=cv2.INTER_LINEAR)
        frame = self.crop.crop_center(frame, up=0.1, down=0.5, left=0.25, right=0.25)
        return frame
    ...
    def get_init_state(self):
        ...
        return self.reshape(self.prev_sim_frame[..., ::-1]), self.reshape(self.sim_frame[..., ::-1]), speed
    ...
    def get_next_state(self, predicted_course=0.):
        ...
        if interv:
            ...
            return self.reshape(self.sim_prev_frame[..., ::-1]), self.reshape(self.sim_frame[..., ::-1]), speed, None

        # increase the frame index
        self.sim_prev_frame = self.sim_frame
        self.sim_frame      = next_sim_frame

        return self.reshape(self.sim_prev_frame[..., ::-1]), self.reshape(self.sim_frame[..., ::-1]), next_location['speed'], rel_course
